...
//	PRVecY = -((( 2.0f * mouseY) / ClientHeight) - 1 ) / camProjection(1,1);
	...
	XMStoreFloat3x3(&camProjection4, camProjection);
...
//	loadSMInfo.MiscFlags = D3D11_RESOURCE_MISC_TEXTURECUBE;
	...
	for(int j = 0; j < rows; j++)
	{
		for(int k = 0; k < rows - j; k++)
		{
			...
			Translation = XMMatrixTranslation( bottleXPos + (float)k + (float)j*0.5f, (float)j*2.25f + 1.0f, bottleZPos);
...
void UpdateScene(double time)
{
	...
	if(bottleFlying)
	{
		...
		tempBottlePos = XMVector3TransformCoord(tempBottlePos, thrownBottleWorld) + (thrownBottleDir * (float)time * 10.0f);
		Rotation = XMMatrixRotationY(3.14f);
